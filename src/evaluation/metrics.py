"""
Модуль для расчета и сохранения метрик моделей обнаружения объектов
"""
...
class YOLODetectionRateCalculator:
    ...
    def calculate_detection_rate(self,
        ...
        # Выводим результаты
        self._print_summary(detection_rate, total_images, detected_images, results)
    ...
    def _print_summary(self, detection_rate: float, total: int, detected: int, results: List):
        ...
        if total > 0 and len(results) > 0:
            avg_gt = np.mean([r["gt_objects"] for r in results])
            avg_pred = np.mean([r["pred_objects"] for r in results])
    ...
    def analyze_errors(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "avg_gt_missed": float(missed["gt_objects"].mean()) if len(missed) > 0 else 0,
            "avg_pred_missed": float(missed["pred_objects"].mean()) if len(missed) > 0 else 0,
            "avg_matched_missed": float(missed["matched"].mean()) if len(missed) > 0 else 0
        }
...
class MetricsCalculator:
    """Класс для расчета и сохранения стандартных метрик YOLO"""
    
    def __init__(self, output_dir: str = "/content/metrics_results"):
        """
        Args:
    ...
    def extract_from_yolo_results(self, results, experiment_name: str) -> Dict[str, Any]:
        """
        Извлекает метрики из результатов YOLO
        
        Args:
            results: результаты валидации YOLO
        ...
        # Общие метрики
        # Безопасно вычисляем средний F1
        f1_values = [v for v in box.f1 if v is not None]
    ...
    def save_metrics(self, metrics: Dict[str, Any], filename: Optional[str] = None) -> Path:
        ...
        Args:
            ...
            filename: имя файла (если None, используется experiment из метрик)
        ...
        if filename is None:
            ...
            filename = f"{experiment}.json"
        
        # Убеждаемся, что filename имеет расширение .json
    ...
    def compare_experiments(self, metrics_files: List[str]) -> Dict[str, Any]:
        """
        Сравнивает несколько экспериментов
        
        Args:
            metrics_files: список путей к JSON файлам с метриками
        ...
        for filepath in metrics_files:
            ...
            comparison[exp_name] = {
                "mAP50": metrics["mean"]["mAP50"],
                "mAP50-95": metrics["mean"]["mAP50-95"],
                "precision": metrics["mean"]["precision"],
                "recall": metrics["mean"]["recall"]
            }
        
        # Находим лучший эксперимент по mAP50-95
        if comparison:
            best_exp = max(comparison, key=lambda x: comparison[x]["mAP50-95"])
            comparison["best_experiment"] = {
                "name": best_exp,
                "metrics": comparison[best_exp]
            }
    ...
    def print_summary(self, metrics: Dict[str, Any]):
        ...
        print("\n" + "="*50)
        print(f"📊 Сводка метрик для эксперимента: {metrics['experiment']}")
        print("="*50)
        
        mean = metrics["mean"]
        print(f"\n📈 Общие метрики:")
        print(f"  mAP50:     {mean['mAP50']:.4f}")
        print(f"  mAP50-95:  {mean['mAP50-95']:.4f}")
        print(f"  Precision: {mean['precision']:.4f}")
        print(f"  Recall:    {mean['recall']:.4f}")
        print(f"  F1:        {mean['f1']:.4f}")
        
        print(f"\n📊 Метрики по классам (топ-5 по AP50-95):")
        # Сортируем классы по AP50-95 и показываем топ-5
        sorted_classes = sorted(
            [(cls, cls_metrics["ap50_95"]) for cls, cls_metrics in metrics["per_class"].items() 
             if cls_metrics["ap50_95"] is not None],
            key=lambda x: x[1],
            reverse=True
        )
        for cls_name, ap in sorted_classes[:5]:
            print(f"  {cls_name:20s}: AP50-95 = {ap:.4f}")
        
        if len(sorted_classes) > 5:
            print(f"  ... и еще {len(sorted_classes) - 5} классов")
    
    def export_to_dataframe(self, metrics: Dict[str, Any]) -> pd.DataFrame:
        """
        Экспортирует метрики в pandas DataFrame
        
        Args:
            metrics: словарь с метриками
            
        Returns:
            DataFrame с метриками по классам
        """
        rows = []
...
# Для обратной совместимости импортируем основные классы на верхний уровень
__all__ = ['MetricsCalculator', 'YOLODetectionRateCalculator']